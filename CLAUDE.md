...
- **课程内阅读顺序**：统一为「速成 → 往年题 → 分讲知识点 → 作业」。用 `00-exam-cram.md` 放速成；用 `01-past-exams/` 文件夹集中逐卷真题，其 `_index.md` 只写展示标题「往年题」、正文留空，卷内按时间从新到旧编号；知识点从 `02-` 起连续编号，作业接在知识点之后。没有源材料的类别只保留空目录，不补造内容。
