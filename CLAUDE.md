...
- **职业相关**（实习、求职、职场反思、行业观察）→ `career`。实习面经与秋招面经分别放在并列的 `internship-interviews/`、`autumn-recruitment-interviews/` 中，每家公司一篇文章。
...
- **当前覆盖**：大一、大二的非水课与已经归档的大三课程均采用统一课程结构；源目录没有真题或作业时保留空文件夹，不补造材料。
...
- **已经发布的文章若因重新编号或移动目录而改 URL**，必须同时在 `astro.config.mjs` 的 `redirects` 中保留旧路径到新路径的跳转，并在构建产物中核对旧路径生成了 redirect 页面，避免外链和搜索结果变成 404。
...
- **语言**：正文以**中文**为主。
- **作者视角（硬规则）**：凡以 owner 名义发布的博客、课程文章和个人文档，都把 agent 视为 owner 的写作分身，默认以 owner 本人的第一人称“我”写作；成稿中不得把 owner 写成 `Lee`、`owner`、“作者”“提交者”等第三方，也不要让“本文 / 本篇”代替“我”作判断、核对或补造。纯客观知识陈述、数学推导中的“我们”、题面 / 引文原话以及确属他人的材料不必机械改写。第一人称不等于可以虚构，个人经历、态度和判断必须来自 owner 已提供的事实或可核验材料。
...
- **作业逐次拆分与答案折叠**：一份源作业或一个独立大作业对应一篇文章，不把整学期作业汇总成一篇。原题、实验要求或项目要求放在折叠块外，解答、提交内容、参考实现与运行结果使用 `<details class="exam-answer">` 默认折叠；实验和项目的 `<summary>` 可以写“查看提交内容与实现”，不机械称为“答案”。若材料只有作答、无法唯一恢复原题，必须明确标注“根据作答还原”，不得补造题面。
- **面经的问答展示**：实习与秋招面经沿用同一展示格式：`description` 写面试结果，正文按“轮次（面试日期）”分节，节内用编号列表组织盘项目、八股、算法题等，具体追问放在对应项内。题目使用正常字体；owner 的回答或现场复盘放在对应题目内的 Markdown 引用块中，首行统一写 `**我的回答**`，回答原文保留斜体。引用块直接复用全站 blockquote 样式，始终展开，不使用真题专用的折叠答案组件。
