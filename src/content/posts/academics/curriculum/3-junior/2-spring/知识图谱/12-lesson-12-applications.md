---
title: "第十二讲 · 知识图谱应用"
description: "串联知识图谱问答、时序推理、可解释推荐、视觉推理与医疗、商业和安全领域案例"
date: 2026-06-25
updated: 2026-08-23
tags: ["知识图谱", "数学"]
---

<!-- markdownlint-disable MD031 MD032 MD036 MD040 -->

> 对应 PPT：第12讲（应用1 + 应用2）
> 重点：问答（KBQA）3 大方法、推荐系统、视觉/跨模态、行业应用。

---

## 1. 知识图谱问答（KBQA）总览

### 1.1 问答系统 4 大历史阶段

| 阶段 | 时间 | 代表 |
|---|---|---|
| 基于模板的问答专家系统 | 1960~ | 早期专家系统 |
| 基于信息检索的问答 | 1990~ | 关键词匹配 + 信息抽取 + 浅层语义 |
| 基于社区的问答 | 2000~ | 百度知道、知乎（依赖网民贡献） |
| 基于知识图谱的问答 | 2010~ | Siri、度秘、Evi、WolframAlpha |

### 1.2 问答形式（3 类）

| 形式 | 特点 |
|---|---|
| **一问一答** | 单轮、单答案 |
| **交互式问答** | 多轮对话、上下文依赖 |
| **阅读理解** | 基于给定文档回答 |

### 1.3 问答系统基本组件

```
自然语言问题 → 问题分析 → 数据匹配 → 构造 query → 答案检索和评估 → 答案
   ↓              ↓           ↓           ↓              ↓
 linguistic     index       ontology    dataset        score
 tools/resources
```

### 1.4 KG 问答的核心挑战

> **将自然语言表达映射为知识图谱元素词汇**，解决**语义偏差**（歧义性、模糊表达）。

例："苹果有哪几个超过两万的电子产品？"——"苹果"指公司，"两万"指价格阈值。

---

## 2. 基本概念（期末爱考辨析）

### 2.1 问句短语

- **Wh-words**：who / what / which / when / where / why / how
- **Wh-words + 名词/形容词/副词**：which party / which actress / how long / how tall

### 2.2 问题类型（必背 8 类）

| 类型 | 例子 |
|---|---|
| **谓词型** | "Who was the first man in space?" / "How far is Earth from Mars?" |
| **列表型** | "Give me all cities in Germany." |
| **最高级型** | "What is the highest mountain?" |
| **对错型** | "Was Margaret Thatcher a chemist?" |
| **观点型** | "What do most Americans think of gun control?" |
| **方法型** | "How do I make a cheese cake?" |
| **解释型** | "Why did the revenue of IBM drop?" |
| **关联型** | "What is the connection between Barack Obama and Indonesia?" |
| **比较型** | "What is the difference between impressionism and expressionism?" |

### 2.3 答案类型（必背）

| 类型 | 例子 |
|---|---|
| **事实型-Entity** | event, color, animal |
| **事实型-Human** | group, individual |
| **事实型-Location** | city, country |
| **事实型-Numeric** | count, distance |
| **事实型-Temporal** | date, time |
| **摘要型** | Abbreviation |
| **描述型** | Description |
| **解释型** | Explanation（"How..."） |
| **证据型** | Justification（"Why..."） |

### 2.4 问题主题

- 例："What is the height of Mount Everest?" → **(geography, mountains)**

### 2.5 领域类型

- **开放域 vs 特定域**
- **数据类型**：文本 / 图片 / 音频 / 视频
- **多模态问答 / Visual QA**

---

## 3. KG 问答 3 大主流方法（必背）

| 方法 | 思路 | 优点 | 缺点 |
|---|---|---|---|
| **基于模板** | 手工 SPARQL 模板匹配 | 响应快、准确率高、可回答复杂问题 | 模板库庞大、与真实问题难匹配 |
| **基于语义解析** | 问题 → 逻辑表达式 → SPARQL | 可回答复杂问题（如时序） | 需语义解析器，规则制定繁琐 |
| **基于深度学习** | LSTM/Attention/Memory Network | 无需人工模板、端到端 | **不包含聚类操作，时序性问题无法回答**（PPT 原话） |

---

## 4. 基于模板的方法

### 4.1 基本思路

理解问题需要 3 部分：
- **语义结构**（与数据集无关）：who → `SELECT ?x WHERE { ... }`
- **领域无关词汇**：the most N、more than i N
- **领域相关词汇**：具体实体、关系、属性

### 4.2 TBSL（Template-Based SPARQL）架构

TBSL 是**模板方法**的经典实现，3 步：

**Step 1: 模板生成 — Linguistic Processing**
- 词性标注
- 用语法规则表示问句
- 区分 domain-dependent / domain-independent 词汇
- 将语义表示转化为 SPARQL 模板

**Step 2: 模板实例化 — 概念识别和关系检测**
- **资源（resources）和类（classes）**：
  - 用 WordNet 定义同义词
  - 计算字符串相似度
- **属性标签（property labels）**：与模式库中自然语言表示比较
- 最高排位的概念填充槽位

**Step 3: 查询排序和选择**
- 每个实体按字符串相似度打分
- 模板分值 = 多个槽位实体的平均打分
- **检查 type 一致性**：`?x rdf:type <class>` 与 `p` 的 domain/range 是否一致
- 返回打分最高的查询

### 4.3 经典例题：Who produced the most films?

**SPARQL 模板**：
```sparql
SELECT DISTINCT ?x WHERE {
   ?x ?p ?y .
   ?y rdf:type ?c .
}
ORDER BY DESC(COUNT(?y)) OFFSET 0 LIMIT 1
# ?c CLASS [films]
# ?p PROPERTY [produced]
```

**实例化**：
- ?c = `<http://dbpedia.org/ontology/Film>`（Score 0.76）
- ?c = `<http://dbpedia.org/ontology/FilmFestival>`（Score 0.60）
- 选 Film（高分）

### 4.4 复杂问题处理

- **依存树重写**：并列连词、关系从句
- **子问题回答**：匹配模板 → 实例化 → 查询 → 排序 → 返回列表
- **答案拼接**：子查询打分 `1/r`（r = 位置），取交集，选**不为空 + 组合后得分最高**的 query 组合

---

## 5. 基于语义解析的方法

### 5.1 4 步流程

```
问句 → 语义解析 → 语义表示 → 资源映射 / Logic Form → 候选答案生成 → 排序 → 答案
```

### 5.2 两种语义表示

| 表示方法 | 特点 |
|---|---|
| **基于符号的表示** | 形式化、可解释（如 λ-calculus） |
| **基于分布式的表示** | 向量化、可融合 DL |

### 5.3 SP vs IR（重要区别）

| 方法 | 思路 | 适用 |
|---|---|---|
| **Semantic Parsing (SP)** | 问句 → 逻辑表达式 → 查询 | 复杂问题 |
| **Information Retrieval (IR)** | 问句 → 候选 → 排序 | 简单问题 |

### 5.4 语义解析器训练

- 数据：**问题-答案对（QA pairs）**
- 训练一个**语义解析器**（神经网络 / 概率模型）
- 学"问句 → 逻辑表达式"的映射

---

## 6. 基于深度学习的方法（重要）

### 6.1 主要模型

| 模型 | 思想 |
|---|---|
| **LSTM** | 序列建模 |
| **Attention Model** | 关注重要部分 |
| **Memory Network** | 外部记忆 + 推理 |

### 6.2 优缺点（PPT 原话）

> **优点**：
> - 无须像模板方法那样**人工编写大量模板**
> - 也无须像语义解析方法那样**制定繁琐的语义规则**
>
> **缺点**：
> - 深度学习方法通常**不包含聚类操作**
> - 因此**时序性问题无法回答**

### 6.3 KEQA / EmbedKGQA / DSSAGN（简单 vs 复杂）

| 模型 | 适用 | 核心 |
|---|---|---|
| **KEQA** | 简单问句（**一阶关系**） | 知识表示学习嵌入 + 简单问答 |
| **EmbedKGQA** | 复杂问句 | BERT 编码复杂问句 + KG 嵌入 |
| **DSSAGN** | 复杂问句 | **句法 + 语义信息 + 增强 GNN** |
| **CF-KGQA** | 复杂问句 | **因果关系**纳入多跳问答 |

### 6.4 时序问答 5 个模型（期末可能考）

| 模型 | 核心创新 |
|---|---|
| **TwiRGCN** | GCN + **时间加权机制** + **门控机制** |
| **CRONKGQA** | 时序知识表示学习（嵌入含时间信息） |
| **TempoQR** | 时间嵌入表示 + 实体感知 |
| **CTRN** | 捕捉**隐式时序特征** + 关系表示 |
| **EXAQT** | 挖**KG 密集子图** + 微调 BERT 增强事件 |
| **Prog-TQA** | **大模型 ICL** + 组合时序约束 |
| **GenTKGQA** | 子图检索 + LLM 生成（LLM 双阶段） |

---

## 7. 推荐系统

### 7.1 传统 vs KG 推荐

| 类型 | 思路 | 局限 |
|---|---|---|
| **传统推荐** | 仅利用用户-物品历史 | 缺乏可解释性、个性化弱 |
| **基于 KG 推理的推荐** | KG 提供**多样化、精确、可解释**的推荐 | 需要构建领域 KG |

### 7.2 KG 推理挖掘用户喜好

| 模型 | 核心 |
|---|---|
| **KGCN** | **图卷积网络** + 聚合邻居信息 |
| **KGNCF-RRN** | **改进的 RRN** + 关系路径编码（多步） |
| **KGECF** | 用户-物品交互建模为**单一关系 KG** + 表示学习 |

### 7.3 KG 推理增强推荐可解释性

| 模型 | 核心 |
|---|---|
| **PGPR** | **强化学习** + 多步推理 + 软奖励策略 |
| **KPRN** | 组合 KG 实体和关系嵌入 |
| **CogER** | 模仿**人类认知过程**（系统1 + 系统2） |
| **KRRL** | **自监督强化学习** + MOOCs 可解释推荐 |
| **KGAT** | **压缩附带信息** + 降低计算成本 + 保持准确性 |
| **RippleNet** | KG 嵌入 + **多跳路径传播** |

---

## 8. 视觉与跨模态

### 8.1 视觉问答（VQA）

> **VQA**：基于**图像 + 知识图谱**回答问题。

| 模型 | 核心 |
|---|---|
| **FVQA** | 数据集格式：图像-问句-答案-支持事实子图 |
| **Graphhopper** | 图像 → 场景图 + KG 路径推理 |
| **大模型 VQA** | LLM + 视觉编码器（GPT-4V 等） |

### 8.2 跨模态检索

| 模型 | 核心 |
|---|---|
| **KCR** | ResNet 图像特征 + 改进 BERT 文本特征 |
| **MMRG** | 文本 KG + 视觉 KG + 类似 BERT 掩码 |
| **IRGR** | 图像/文本/实例相似度矩阵 + KNN 关系图谱 |

### 8.3 场景理解

| 模型 | 核心 |
|---|---|
| **GB-Net** | Faster R-CNN 初始化场景图 + ConceptNet/WordNet |
| **HiKER-SGG** | **层次化 KG** + 初始场景图桥接 |
| **CGR** | 不同路径匹配 + 检索 + 选择性路由 |
| **COACHER** | **常识 KG** + 图挖掘模块 |

---

## 9. 垂直领域应用

### 9.1 医疗领域

- **KGNN**（IJCAI 2020）：药物相互作用预测
- 整合电子病历 + 医学 KG + KGE 推理 → 安全药物推荐

### 9.2 商业领域

- **OpenBG**（ICDE 2023）：**亿级多模态商业 KG** + 视觉-语言基础模型预训练 → 商品分类预测
- **KAPR**（KBS 2022）：多特征推理 + 动态策略网络 → 多步推理预测产品替代/互补关系
- 多种 KGE 技术 → 发现客户需求与产品的隐含关系
- 异常检测：KG 嵌入 + ML

### 9.3 信息安全领域

- **MDATA** + **AMIE** 规则学习 → 发现新网络中的知识
- 基于 **TransH** 的 KG 推理 → 自动识别攻击模式

### 9.4 其他领域（已知案例）

- 电商（阿里、京东）、餐饮娱乐（美团）、学者（AMiner）、中医药、天眼查（已在第 1 讲讲过）

---

## 10. 3 大方法对比总结（必背）

| 维度 | 模板 | 语义解析 | 深度学习 |
|---|---|---|---|
| **响应速度** | **快** | 中 | 慢 |
| **准确率** | 高 | 高 | 一般 |
| **复杂问题** | 可回答 | **可回答**（如时序） | **不可回答**（无聚类） |
| **人工成本** | 高（模板库） | 高（语义规则） | 低 |
| **可解释** | 高 | 高 | 低 |

> **期末最爱考**：深度学习的"无聚类 → 时序不可回答"。

---

## 11. 本章脑图

```
知识图谱应用
├── KBQA（KG 问答）
│   ├── 4 阶段：模板 → IR → 社区 → KG
│   ├── 3 形式：一问一答 / 交互 / 阅读理解
│   ├── 3 大方法（必背对比）
│   │   ├── 模板（TBSL）
│   │   │   └── Step 1 生成 / 2 实例化 / 3 排序
│   │   ├── 语义解析（SP/IR）
│   │   └── 深度学习（KEQA/EmbedKGQA/...）
│   ├── 简单问句（KEQA）
│   ├── 复杂问句（EmbedKGQA/DSSAGN/CF-KGQA）
│   └── 时序问句（TwiRGCN/CRONKGQA/TempoQR/...）
│
├── 推荐系统
│   ├── 挖掘喜好：KGCN / KGNCF-RRN / KGECF
│   ├── 可解释：PGPR / KPRN / CogER / KRRL / KGAT / RippleNet
│
├── 视觉与跨模态
│   ├── VQA：FVQA / Graphhopper / 大模型
│   ├── 跨模态检索：KCR / MMRG / IRGR
│   └── 场景理解：GB-Net / HiKER-SGG / CGR / COACHER
│
└── 垂直领域
    ├── 医疗：KGNN / KGE 推理
    ├── 商业：OpenBG / KAPR
    └── 信息安全：MDATA + AMIE / TransH
```

---

## 12. 课件补全与勘误

### 12.1 问题类型一共九类

前文标题写“必背 8 类”，但紧接着的表格实际列出了九类。按 PPT 应记为：

1. 谓词型
2. 列表型
3. 最高级型
4. 对错型
5. 观点型
6. 方法型
7. 解释型
8. 关联型
9. 比较型

其中前四类在课件中归入事实型问题，其余五类分别需要观点总结、步骤、原因、关系或差异等不同回答策略。“问题类型”决定如何处理问题，“答案类型”则约束最终输出是实体、人物、地点、数值、时间还是一段解释，二者不要混为同一分类。

### 12.2 TBSL 例题的方向冲突与标准写法

![TBSL 从问句生成并实例化 SPARQL 模板](/images/knowledge-graph/tbsl-architecture.png)

课件的 “Who produced the most films?” 在不同页面出现了方向冲突：一处写作 `?film dbo:producer ?producer`，另一处却写作 `?producer dbo:producer ?film`。按 DBpedia 的 `dbo:producer` 方向，应由作品指向制片人，即：

```text
Film --dbo:producer--> Producer
```

若要求严格、可执行的 SPARQL 聚合查询，还应显式按制片人分组：

```sparql
PREFIX dbo: <http://dbpedia.org/ontology/>

SELECT ?producer (COUNT(DISTINCT ?film) AS ?filmCount)
WHERE {
  ?film a dbo:Film ;
        dbo:producer ?producer .
}
GROUP BY ?producer
ORDER BY DESC(?filmCount)
LIMIT 1
```

理解这个例子时分三步：`?film a dbo:Film` 限定作品类型，`dbo:producer` 找制片人，`GROUP BY + COUNT + ORDER BY` 才表达“制作电影最多”。不能只看自然语言中“谁”在句首，就把图上的边反过来。

### 12.3 深度学习局限只适用于课件讨论的早期方法

前文“深度学习不能回答复杂或时序问题”的绝对表述，需要限定为 PPT 在介绍早期 LSTM、Attention、Memory Network 方法时的总结：这些方法通常面向简单、单边关系问题，没有为 `first`、`before`、`after` 等约束设计专门操作，因此容易忽略时序语义。它不是对所有深度学习 KBQA 的永久结论。

同一门课后续正式介绍了七个专门的时序问答模型，正好说明这一限制可以通过显式时间建模、子图推理或程序生成来缓解：

| 模型 | 如何处理时间 |
|---|---|
| **TwiRGCN** | 时间加权 GCN，并用门控判断答案是实体还是时间 |
| **CRONKGQA** | 学习含时间信息的实体和关系嵌入，分别给实体与时间候选打分 |
| **TempoQR** | 把实体嵌入和时间嵌入组合成时间感知问句表示 |
| **CTRN** | 捕捉隐式时序特征与关系表示，生成时序关系表示 |
| **EXAQT** | 检索问题相关的密集子图，以 BERT 和 R-GCN 做时间感知节点分类 |
| **Prog-TQA** | 用大模型上下文学习生成程序草稿，再与时序 KG 对齐 |
| **GenTKGQA** | 先检索带时间约束的子图，再让经过指令微调的 LLM 生成答案 |

所以应记成“通用序列模型缺少专门时序操作”，而不是“深度学习天然不能做时序问答”。

### 12.4 问答与预训练模型补全

正式课件还给出了几类正文没有展开的模型：

- **KEPLER**：统一知识嵌入与预训练语言表示，在语言模型目标之外加入知识嵌入目标，使实体描述文本和图谱事实进入同一个表示学习框架。
- **BRGNN**：对复杂多跳问句进行双向推理并生成查询路径；双向信息传播比单向搜索能捕捉更丰富的路径语义。
- **化学领域问答案例 Marie and BERT**：用 BERT 提升问句实体提及链接到化学知识图谱实体的鲁棒性；在多个表示空间中运行多种 KGE 模型得到候选答案，再进行分数对齐和重排序，并结合多步推理处理专业问题。

这三例分别展示了三条路线：把 KG 注入语言模型、在图上学习双向多跳路径、以及把领域实体链接与多模型答案融合起来。

### 12.5 可解释推荐：图、路径与自然语言

![知识图谱通过多跳路径连接用户兴趣与候选物品](/images/knowledge-graph/kg-recommendation.png)

课件中的评论增强推荐不是只建一张图，而是并行处理两种结构：

- **用户—物品交互图**描述点击、购买、评分等协同过滤信号。
- **知识图谱**描述物品、演员、类别等外部语义关系。

模型在两张图上并行执行基于 GCN 的协同过滤，得到推荐结果后，再利用知识图谱嵌入生成“目标用户—中间实体—推荐物品”的路径作为解释；评论文本或语言模型负责把结构化证据转成更自然的说明。交互图回答“行为上像不像”，知识图谱回答“为什么相关”。

**元路径**则是异构图上的类型级路径模板。例如：

```text
用户 --看过--> 电影 --主演--> 演员 --主演的逆关系--> 电影
```

它描述的是一类可复用的语义关系，而不是某几个具体实体组成的单条路径。元路径翻译模型把长而复杂的节点类型、关系类型序列改写成自然语言，例如“推荐这部电影，因为它与用户看过的电影由同一演员主演”，从而让推荐理由更易读。

### 12.6 视觉问答中，图谱提供图像之外的事实

![知识增强视觉问答从图像概念连接到外部知识](/images/knowledge-graph/knowledge-vqa.png)

普通视觉问答只依赖画面内容，知识增强 VQA 还会把检测到的物体、属性和场景链接到外部 KG。FVQA 用“图像—问句—答案—支持事实子图”明确标出证据；Graphhopper 则先构造场景图，再用强化学习搜索多跳路径。二者共同的关键是：答案可能不直接出现在像素里，而要从“图像概念 → 图谱实体 → 背景事实”推出来。

### 12.7 方法对比表的证据边界

前文总结表中“语义解析速度中等、深度学习慢”“深度学习准确率一般”等等级没有统一实验设置或数值来源，不能当作课件证明的普遍排序。PPT 明确给出的定性判断是：模板查询响应快、准确率较高，但模板设计成本高；语义解析能处理较复杂问题，但规则工程量大；早期深度学习方法自动化程度高，但当时对复杂和时序问题支持不足。

真实性能取决于候选空间、图谱规模、模型参数量、是否需要多跳搜索以及缓存策略。考试若要求按课件对比，就写上述定性优缺点；不要补造一个跨系统通用的速度或准确率等级。
