...
> **先看勘误**：下面保留了原复习稿的全部内容；其中 TransE 可用 L1 或 L2 距离，并非固定 L2；`DisMult` 应为 `DistMult`；R-GCN 负责得到实体表示，最终三元组分数还需另接解码器；RPJE 的综合式第一项应是距离标量而不是裸向量。文末会逐项校正并补齐后半段课件。
...
## 11. 先把任务看懂：KGE 到底在解决什么

知识图谱补全通常给定三元组中的两个实体和一条关系，预测缺失项：

$$
(h,r,?)\qquad\text{或}\qquad(?,r,t)
$$

KGE 先把符号实体和关系变成可计算的数值表示，再用打分函数衡量候选三元组是否可信。训练时要让真实三元组得分更好、负样本得分更差；测试时则把全部候选实体排序，用 MR、MRR 和 Hits@n 评价正确答案的位置。

理解模型时始终问三个问题：

1. **表示什么**：实体、关系分别是向量、矩阵、分布，还是文本表示？
2. **怎样交互**：平移、双线性乘积、卷积、消息传递，还是语言模型编码？
3. **补了哪块信息**：只看当前三元组，还是还用了路径、规则、类型、文本、图像和时间？

## 12. 基础模型的统一口径与勘误

### 12.1 TransE：L1 和 L2 都可以
...
下标 $1/2$ 表示实现时可以选择 L1 或 L2 范数。真正不变的是“正确三元组的平移距离要小”，而不是必须使用 L2。
...
$$
(h,r,t)\longrightarrow(h',r,t)\quad\text{或}\quad(h,r,t')
$$

但随机替换会遇到两类问题：

- 替换结果可能本来就在知识图谱里，属于**假阴性**；训练前应过滤已知真三元组。
- 对 1-N、N-1 关系，头尾两侧的错误概率不一样；Bernoulli 负采样会按关系映射特性调整替换哪一侧。

### 12.3 四类模型不能只背名字

| 路线 | 核心交互 | 代表模型 | 主要代价 |
|---|---|---|---|
| 平移 | $h+r\approx t$ | TransE / H / R / D | 简洁，但复杂关系需要扩展 |
| 张量分解 | 双线性或多线性交互 | RESCAL / DistMult / ComplEx | 表达强，参数或对称性受约束 |
| 神经网络 | 非线性变换 | NTN / ConvE / ConvKB / KG-BERT | 计算量较大 |
| 图神经网络 | 沿关系聚合邻居 | R-GCN / KGAT / KBAT | 依赖邻域采样与解码器 |

这里原稿中的 `DisMult` 是拼写错误，应为 `DistMult`。R-GCN 本身主要编码邻域，常与 DistMult 等解码器配合；所以原来的 $\operatorname{score}(h,r,t)$ 只是占位写法，不能当作一个完整打分函数。

### 12.4 RPJE 的三个量必须都是标量

RPJE 联合利用当前三元组、路径和规则。推理时的综合量应写成距离形式，例如：

$$
Q(h,r,t)=\lVert\mathbf h+\mathbf r-\mathbf t\rVert
+\alpha_1\sum_{p}\sum_{\mu_i\in B(p)}
R(p\mid h,t)\mu_i\lVert\mathbf p-\mathbf r\rVert
$$

原稿把第一项写成裸向量 $\mathbf h+\mathbf r-\mathbf t$，无法与后面的标量距离直接相加。课件的路径搜索允许长度不超过 3；实验中“最大长度设为 2 更好”是特定实验结果，两者并不矛盾。

## 13. 给 KGE 增加实体类型：TKRL、AutoETER 与 CAKE
...
## 15. 课件最后一段：知识图谱推理的新进展

### 15.1 强化学习：把找路径改写成 MDP
...
### 15.3 Transformer：KG-BERT 与 CSProm-KG
