...
Wikipedia 分类可以帮助建立概念层次，但它并不天然等于严格本体，仍需要清洗。Redirects 处理别名，Disambiguation 处理同名不同实体，两者不要混用。
...
- 全球电子表格活跃用户 **>40 亿**
- **2019 年口径**：Wikipedia 52 万篇文章中可抽取 **323 万+** 张表
- **有价值信息占比小**：141 亿 HTML 表中只有 **1.54 亿（1.1%）** 是高质量

后一组是 Cafarella 等人 2008 年 WebTables 工作的口径。两组数字不能共用一个年份，但都在说明：网络表数量很大，真正承载高质量关系数据的比例很低。
...
完整的表语义阐释还要继续做关系与类型解释：先恢复题目、题注、列头、行和单元格，再生成候选实体、完成消歧，最后利用列头和同列类型确定谓词并生成三元组。例如“姚明｜上海｜2.26 米”只有在列头被解释为“人物｜出生地｜身高”后，才能可靠生成事实。
...
> **重要性（importance）评价**网页（实体）。这是一个**递归**过程：页面从指向它的页面接收权重，再把自己的权重分给它所指向的页面。

### 6.2 物理意义

- 想象用户在网上**随机浏览**
- 一个网页的 rank = 随机游走达到稳态后，浏览者**处于该页面的长期概率**
- 高质量页面的链接比低质量页面的链接贡献更大
...
初始时五个节点都是 $0.2$。按课件规则，E 把权重平均分给其余四个节点，每个得 $0.05$。一轮更新为：

$$
\begin{aligned}
PR_1(A)&=0.2/3+0.2/3+0.2/2+0.05\approx0.28,\\
PR_1(B)&=0.2/2+0.05=0.15,\\
PR_1(C)&=0.2/2+0.2/3+0.05\approx0.22,\\
PR_1(D)&=0.2/3+0.2/3+0.05\approx0.18,\\
PR_1(E)&=0.2/3+0.2/2\approx0.17.
\end{aligned}
$$

五个结果之和约为 1，与课件图中的 0.28、0.15、0.22、0.18、0.17 一致。

### 6.4 关键规则

- **课件对无外链节点的处理**：该实体的 rank 平均分给图中其他实体
- **物理直觉**：用户"找不到目标"时会"随机跳到别处"
- **实践中**：用阻尼系数 `d`（通常 0.85）控制跳转概率：
  `PR(p) = (1-d)/N + d * Σ PR(q)/L(q)`

标准实现通常把 dangling node 的权重均匀分给**全部 $N$ 个节点**，并加上随机跳转项。考试若明确给出课件的“分给其他节点”规则，就按题面计算。
...
> 2. 大模型能否实现包装器？
>    答：可以，LLM（如 GPT-4）能根据 HTML + 抽取需求直接生成结构化数据。是否合适取决于页面模板、校验要求、调用规模和延迟预算：固定模板下传统包装器通常更稳定、成本更低；页面差异较大时 LLM 更灵活，但仍需结构约束和结果校验。

课件还布置了两项实践：以大作业小组为单位提交领域本体模型和有向图可视化；针对小组领域的开放网页手工构造包装器，同时提交原始网页与实体、关系、属性抽取结果。
