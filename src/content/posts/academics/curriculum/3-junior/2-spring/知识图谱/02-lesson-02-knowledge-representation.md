...
前文发展表中的 1934、1937、1966、1998、2004 等精确年份属于整理时补充的历史节点；本讲 PPT 本身按 `-1960`、`1960—1970`、`1970—1980`、`1980—1990`、`1990—2000`、`2000—2010`、`2013—` 的阶段展示。两者可相互参照，但不要把精确年份误认成期末课件原表。

### 14.1 本体五元组：Function 不能漏

> **勘误**：前文把本体写成了“概念、关系、实例、属性、公理”。课件采用的五元组是**概念、关系、函数、公理、实例**，可以记作
>
> $$
> O=(C,R,F,A,I).
> $$
...
| 元素 | 回答的问题 | 例子 |
|---|---|---|
| 概念 $C$ | 世界里有哪些类别？ | 人、汽车、课程 |
| 关系 $R$ | 类或实例如何关联？ | `part-of`、`kind-of`、`instance-of`、`attribute-of` |
| 函数 $F$ | 前 $n-1$ 个元素能否唯一确定第 $n$ 个？ | `Father-of(x,y)` 中，给定 $x$ 后用函数关系约束 $y$ |
| 公理 $A$ | 哪些断言恒真、可用于推理？ | 子女关系传递得到子孙关系 |
| 实例 $I$ | 具体对象是谁？ | 姚明、北航、知识图谱课 |

“属性”仍然是建模中的重要概念，但在这份课件的五元组定义里，它属于关系语义的一种，不应拿来替换 Function。

### 14.2 语义网技术栈是一条贯穿链

![语义网技术栈](/images/knowledge-graph/semantic-web-stack.png)

*从全局标识和文档语法出发，逐层得到图数据、模式、公理、查询与可信应用。*

语义网的目标可以概括为“知识的数据化、数据的知识化”：给原本只负责展示的 Web 文档补上机器可理解的语义，使数据可以跨应用交换、组合并支撑推理。

| 层次 | 负责什么 | 不负责什么 |
|---|---|---|
| URI / IRI | 给网络中的资源一个全局标识 | 不直接说明资源含义 |
| XML、XML Schema | 提供树状文档语法和数据类型 | 标签名本身没有共享语义 |
| RDF | 用主语—谓词—宾语表达有向图事实 | 只提供很基础的语义约束 |
| RDFS | 定义类、属性、定义域、值域和子类层次 | 不能充分表达不相交、基数等复杂公理 |
| OWL | 在 RDFS 上增加等价、不相交、互逆、基数等公理 | 表达越强，推理成本通常越高 |
| SPARQL | 用图模式匹配查询 RDF 数据 | 它是查询语言，不是新的知识模型 |

这条链可以用一个商品例子走通：URI 标识“华为 FreeBuds Pro 2”，RDF 描述它的品牌和功能，RDFS 定义“耳机”是“电子产品”的子类，OWL 规定“同品牌”“同功能”等更强语义，SPARQL 再查询具有相同品牌或功能的商品。

课件技术栈的顶层还放置 RIF、加密、证明、信任和用户界面等尚未全部标准化或持续演进的能力。它们回答的不是“事实怎么写”，而是规则怎样交换、推理结果怎样证明、来源是否可信以及人怎样使用这些知识。

![RDF 电商商品表示与推理示例](/images/knowledge-graph/rdf-ecommerce-example.png)

*不同商品通过共享 URI、词汇与关系形成可查询、可推理的商品图。*

### 14.3 OWL 1 与 OWL 2 不要混成四级台阶

前文把 OWL Lite、OWL DL、OWL Full、OWL 2 并称“四层次”，方便记忆但标准关系并非如此：

- **OWL Lite、OWL DL、OWL Full** 是 OWL 1 的三个子语言；表达能力逐渐增强，而 OWL Full 不保证可判定。
- **OWL 2** 是后续版本，不是排在 OWL Full 后面的“第四档”。课件重点介绍三个面向不同工程场景的 OWL 2 Profile。

| OWL 2 Profile | 主要目标 | 适合的直觉 |
|---|---|---|
| **OWL 2 QL** | 基于本体的数据查询 | 便于把本体查询改写到底层关系数据库 |
| **OWL 2 EL** | 大型术语体系的分类与本体推理 | 概念很多、层次很深时仍需高效推理 |
| **OWL 2 RL** | 用规则系统实现可扩展推理 | 规则化实现，推理复杂度可控在多项式时间 |

### 14.4 OWL 示例的语法边界

> **勘误**：前文“OWL 核心构造”的代码块混用了 Turtle 三元组、描述逻辑符号和自然语言简写，因此不能把整块复制到 Turtle 解析器。尤其互逆属性的正确词汇是小写开头的 `owl:inverseOf`，而不是 `owl:InverseOf`。

下面才是可直接辨认为 Turtle 的一个最小片段：
...
描述逻辑中的 $\sqcap$、$\exists$、$\geqslant$ 等概念表达式需要转换成 OWL 对应的 RDF 列表和空白节点结构，不能直接塞进 Turtle 主谓宾位置。

课件“对称性”的示例文字旁写了 `owl:TransitiveProperty`，这会声明传递性而非对称性，疑似幻灯片笔误。配偶关系应使用 `owl:SymmetricProperty`；祖先关系才适合 `owl:TransitiveProperty`。不能因为标题写“对称性”就忽略代码中的语义差异。

### 14.5 JSON-LD：让普通 JSON 接上共享语义

普通 JSON 的 `name`、`jobTitle` 只是局部字段名；机器不知道两个系统中的 `name` 是否同义。JSON-LD 用 `@context` 把字段链接到共享词汇，并用 `@id` 标识资源：
...
它仍保持 JSON 的开发体验，但其数据模型能够解释为 RDF 有向图，也可以通过相应 API 在 JavaScript、Python、Ruby 等环境中转换处理。

### 14.6 分布式表示不只有实数点向量

符号图谱适合解释和规则推理，却面临长尾稀疏、专用图算法难迁移等问题。分布式表示把实体和关系嵌入低维数值空间，使相近语义在空间中更接近，并可与神经网络特征融合。课件列出四类表示空间：

| 数值空间 | 能表达的侧重点 |
|---|---|
| 实数向量或张量 | 结构简单，便于一般数值计算 |
| 复数向量空间 | 更自然地建模对称与反对称关系 |
| 高斯分布 | 用均值和方差表达实体、关系及其不确定性 |
| 流形空间 | 建模球面、曲面等非线性几何结构 |

TransE 的 $\mathbf h+\mathbf r\approx\mathbf t$ 只是其中最容易理解的起点，并不代表所有知识图谱表示学习模型都采用平移假设。

它的训练直觉必须同时看正、负三元组。对正确事实“姚明出生于上海”，希望
...
尽可能小；对错误候选“姚明出生于北京”，希望同一距离尽可能大。只把正例拉近而不构造负例，所有向量可能塌缩到一起，模型无法学会区分事实真假。

### 14.7 本体工程、LLM 与 Protégé

本体工程结构化、可解释且支持逻辑推理，但人工抽取和维护成本高；LLM 泛化和生成能力强，却可能产生幻觉，也不能天然保证逻辑一致。课件给出三种结合方式：

课件用四个关键词限定“什么才算本体”：**共享（shared）**表示领域共同认可，而非个人私有定义；**概念化（conceptualization）**表示对客观世界进行抽象；**明确（explicit）**表示概念及约束没有含混定义；**形式化（formal）**表示机器可以处理。缺少共享语义和形式约束的分类目录，不一定构成严格意义上的本体。

1. 让 LLM 协助生成、补全或优化本体，再由专家审核；
2. 用本体中的类型、公理和约束限制 LLM 输出；
3. 让 LLM 检索知识图谱，用结构化证据增强回答。

**Protégé** 是把这些抽象概念落到实践的开源本体建模工具。以教育领域为例，可以在其中建立 Class 层次、Object Property、Data Property 和 Individual，运行 HermiT 等推理机检查一致性，并导出 OWL。工具的意义不是“画一张分类图”，而是让 URI、类、关系、公理和实例都进入同一套可验证模型。

### 14.8 课件思考题

1. “文化课”“换装”“无人驾驶”等抽象概念不容易枚举固定属性：深度学习该怎样标注、训练，知识图谱又该怎样把类别、上下文和边界条件表示出来？
2. 复杂句子既能写成多元组，也能拆成多个三元组，为什么多数知识表示系统仍偏爱三元组？

第二题可以从统一接口、图结构复用、查询组合和工具生态思考；但拆成三元组也会引入中间节点，并可能丢失原句中的整体事件边界，这正是事件建模仍需单独设计的原因。
