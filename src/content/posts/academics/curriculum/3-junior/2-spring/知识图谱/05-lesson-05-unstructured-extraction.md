...
$$
P(Y | X) = \frac{\exp\left(\sum_j \sum_i w_j f_j(y_{i-1}, y_i, X, i)\right)}{\sum_{Y'} \exp\left(\sum_j \sum_i w_j f_j(y'_{i-1}, y'_i, X, i)\right)}
$$
...
CRF 在所有候选标签序列上做全局归一化，预测也是对整条序列取 `argmax`，而不是每个 token 独立取最大概率。
...
|---|---|---|
| 模型类型 | 生成式 | 判别式 |
| 建模假设与特征 | 对状态转移和观测作较强的条件独立假设 | 直接建模 $P(Y\mid X)$，可灵活使用重叠的输入特征 |
| 特征工程 | 难 | 灵活 |
| NER 效果 | 一般 | **更优** |
...
```text
Pierre → Forward LSTM  → h_forward(Pierre)
Pierre ← Backward LSTM ← h_backward(Pierre)
```

对第 $i$ 个 token，BiLSTM 合并的是**同一位置**的正向和反向状态：

$$
\mathbf h_i=[\overrightarrow{\mathbf h_i};\overleftarrow{\mathbf h_i}].
$$

课件的 Attention BiLSTM 关系分类模型用逐元素求和，即 $\mathbf h_i=\overrightarrow{\mathbf h_i}+\overleftarrow{\mathbf h_i}$。两种方式都合并同一 token 的两个方向，不能把不同 token 的状态相加。
...
- 预测可能不合法，如 `B-PER → I-LOC`：`I-LOC` 不能跟在 `B-PER` 后面
- **CRF 加约束**：学习标签转移矩阵，让无效序列得分变低

BIO 中，`I-X` 前面应是同类型的 `B-X` 或 `I-X`；`B-Y` 表示新实体开始，可以直接跟在另一个实体后面。因此 `B-PER → I-PER → B-LOC` 可以是合法序列，实体之间不强制出现 `O`。

```
Pierre   loves   Paris
B-PER    O       B-LOC
   ↓
路径 "B-PER → O → B-LOC" 得分：31（合法）
路径 "B-PER → I-LOC → B-LOC" 得分：26（非法，CRF 压低）
```
...
> BiLSTM-CRF 是理解序列标注式 NER 的经典基线；实际选型还要考虑预训练 Transformer、跨度分类和生成式方法。
...
这个模型的序列层用 BiLSTM 做实体识别和上下文编码，依存层沿实体之间的依存子树判断关系。联合学习可以**缓解** NER 错误向 RE 传播，但共享表示与联合解码也可能互相干扰，不保证消除误差。

**流水线 vs 联合**：

| 维度 | 流水线 | 联合 |
|---|---|---|
| 错误积累 | **有**（NER 错 → RE 错） | 可缓解传播，仍可互相干扰 |
| 复杂度 | 低 | 高 |
| 效果 | 取决于上游质量 | 常能改善，不保证始终更优 |
...
|---|---|---|
| 模型数 | 多个分类器 | 单一模型 |
| 误差传播 | **有** | 可缓解，但子任务仍可互相干扰 |
| 性能 | 取决于各阶段 | 常能改善，不是无条件更优 |
