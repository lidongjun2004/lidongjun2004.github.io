...
description: "按课程结构串联概念、公式与答题套路，并标清个人复习优先级和课件口径勘误"
date: 2026-06-25
updated: 2026-08-23
tags: ["知识图谱", "数学"]
---
...
> **这篇是个人复习导航，不是真题统计。** 下文的“高频”“必考”“多次考”和三档梯队，都是为了安排复习顺序而做的个人优先级标记。317 页串讲课件本身不能证明某个知识点历年出现过几次，也不能证明某题必考。

同理，下文给出的选择题、简答题、计算题和综合题数量，以及“总计 120 分钟”的时间分配，是个人模拟与应试建议，不是当前材料能够确认的正式试卷结构。使用时可以拿它控制节奏，但不要把它当成考试通知。

阅读正文时还要以这组更正为准：

| 正文中的旧口径 | 准确口径 |
|---|---|
| 5 类一般推理 | 顶层是**演绎、归纳、溯因、类比**四类；统计推理属于归纳推理 |
| KG 推理三路是逻辑、图、统计 | 三路是**基于逻辑、基于图结构、基于表示学习** |
| KGE 有 4 大评价指标 | 课程列出 **MR、MRR、Hits@n** 三类指标；不同 $n$ 的 Hits@n 仍属同一类 |
| TransE 局限是“1-N、对称、组合” | 第 10 讲明确强调 TransE 难以处理 **1-N、N-1、N-N** 复杂关系；“组合”不是该页给出的标准局限 |
| 深度学习 KBQA 无法回答时序问题 | 这是早期通用 LSTM、Attention、Memory Network 方法缺少专门操作时的局限；第 12 讲已有七类时序问答模型 |

此外，正文中第 11 讲的 KGE 改进模型和第 12 讲的问答、推荐、视觉与行业案例来自本课程的**正式课件补充**。它们可以纳入完整复习，但“正式讲过”不等于“已被串讲页证明为高频考点”，两种证据不要混用。
