...
所以 TSK 不是一座孤岛，而是「规则系统、局部模型和集成学习」的交叉点。

## 4. ANFIS：把一次模糊推理摊成五层网络
...
专题最后从模糊系统跳到 Mamba，看似突然，其实在强调同一个方法论：**把已有数学结构嵌进可学习模型**。
...
这里的**状态变量**不是随便挑的中间量，而是一组足以概括系统记忆的最小变量：只要知道当前状态和今后的输入，就能唯一确定系统今后的运动。把它们排成向量 $x$，所有可能的 $x$ 构成**状态空间**。
...
这讲真正想传达的不是几个模型名字，而是：**可解释的先验结构与数据学习并不冲突**。下一讲进入更彻底的连接主义，看神经网络怎样直接从样本学习控制律和序列依赖。
