...
> 符号主义先写规则，连接主义需要样本答案；行为主义再走一步：**没人教、没标准答案，靠「做了看反馈」试错学习——对了奖励就强化，错了惩罚就改**。它最像人和动物从经验中学习。
...
或者用更强但容易验证的充分条件 $\|I-P\Gamma\|<1$。满足时，每个误差模态都像 $0.8^n$ 一样指数衰减到 0。读出精髓：① 增益 $\Gamma$ 要与对象 $P$ 的**逆动态**相似（让 $P\Gamma$ 接近单位映射）；② **不需精确模型**——只要 $\Gamma$ 方向定对、大小合理，误差就被持续压小。这从数学上解释了 ILC「免精确模型」的本质，它要的是**正确的增益方向和范围**。
...
| PPO | 离散 / 连续 | 加**裁剪 Clip** 限制新旧策略更新幅度 | 稳健、常用的通用基线；InstructGPT 曾用于 RLHF |
| SAC | 连续 | **最大熵** RL，奖励 + 策略随机性（熵）→增强探索 | 连续控制中的强基线 |
| TD-MPC | 连续 | **有模型**，学隐变量世界模型 + 短期规划 + 长期价值 | 许多任务上具有较高样本效率 |

记忆主线：AC→A2C→A3C→TRPO→**PPO**（离散、连续动作都可用，更新较稳）；DDPG→TD3→**SAC**（面向连续控制，以最大熵平衡探索与利用）；传统 MPC + 无模型价值→**TD-MPC**。PPO 曾用于 InstructGPT 的人类反馈强化学习，但这不等于当前所有 ChatGPT 或大模型都固定使用 PPO。
