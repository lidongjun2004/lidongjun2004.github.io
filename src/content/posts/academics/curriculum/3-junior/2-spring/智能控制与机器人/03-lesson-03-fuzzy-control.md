...
在线闭环每个采样周期依次执行：读取 $r(k),y(k)$ → 计算 $e(k)=r(k)-y(k)$ 与 $ec(k)=e(k)-e(k-1)$ → 模糊化 → 查规则表得到 $\Delta k_p,\Delta k_i,\Delta k_d$ → 更新当前 PID 参数 → 计算控制输出 → 保存本次误差。这样“模糊推理”才真正落成能实时运行的参数自校正器。
