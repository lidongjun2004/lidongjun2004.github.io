...
课件给出的 Kuhn 定理说明：**每个有限扩展式博弈都存在子博弈精炼 Nash 均衡。**

这个结论解决的是“至少有一个”，不保证均衡唯一。若某些节点上不同动作给行动者相同收益，逆向归纳可能保留多个选择，从而产生多个 SPNE；如果博弈没有真子博弈，SPNE 对普通 Nash 均衡也不会提供额外筛选。

因此，求出一个 SPNE 后仍要检查：题目是否还存在其他并列的后续最优行动，以及这些行动能否组合成其他完整策略。
...
## 十一、要挟诉讼：沉没成本怎样把空话变成承诺

课件用要挟诉讼把“可信承诺”算了一遍。原告先决定是否指控，指控成本为 $c$；随后提出私了金额 $s$。若被告拒绝，原告再决定放弃还是上法庭。上法庭还需支付诉讼成本 $p$，以概率 $\gamma$ 胜诉并获得赔偿 $x$；被告应诉的成本为 $d$。
...
\text{放弃收益}=-c,
\qquad
\text{起诉的期望收益}=\gamma x-c-p.
$$

课件假设 $\gamma x<p$，因此原告到了最后一步会放弃。被告预见到这一点就会拒绝私了，原告再往前倒推也不会发起指控。所谓“我一定告到底”只是离轨威胁。

现在假设原告在提出指控时就不可退还地支付 $p$。拒绝私了以后，放弃与起诉的收益分别变成
...
只要 $\gamma x>0$，上法庭就比放弃好，威胁因沉没成本而可信。被告愿意支付私了金额的上限是上法庭的预期损失 $\gamma x+d$；原告愿意私了则要求 $s\ge\gamma x$，所以私了区间为

$$
s\in[\gamma x,\gamma x+d].
$$

若原告拥有无协商余地的报价权，会要求 $s=\gamma x+d$；当

$$
\gamma x+d>p+c
$$

时，发起指控有利可图，即使案件本身满足 $\gamma x<p+c$，要挟诉讼仍可能发生。被告也可以预付不可退还的辩护费 $f$，把私了区间缩为 $[\gamma x,\gamma x+d-f]$，但这也意味着没有诉讼时会白白承担沉没成本。

这个例子没有说“沉没成本越多越好”，而是说明承诺的机制：先改变未来节点上的收益比较，再通过逆向归纳改变当前行为。
