---
title: "第六讲 · K 均值聚类"
description: "聚类准则、交替优化的 EM 思想、算法步骤、与 GMM 的硬软分配关系、肘部法则；我在文末附上半部分全公式速查"
date: 2026-08-22
tags: ["数学"]
---

K 均值是无监督聚类的代表算法。我按“准则 → 两步交替 → 收敛”的顺序展开，并比较它与 GMM/EM 的关系；文末还附了上半部分的公式速查表。
...
![K 均值在分配样本与重算中心之间交替迭代](/images/pattern-recognition-vision/k-means-iterations.png)

*图：每轮只保证目标函数不增，因此算法会收敛，但可能停在依赖初始中心的局部最优解。*

## 五、与 EM 的关系

两步分别对应 EM 的 E 步（分配责任）与 M 步（更新参数）。区别：**K 均值是硬分配**（$r_{nk}$ 只取 0/1），**GMM + EM 是软分配**（一个点按概率部分属于各类）。当各高斯分量具有相同的球形协方差与先验，并取硬分配或小方差极限时，GMM 的更新会化成 K 均值；因此常把 K 均值看作这类 GMM 的硬分配极限，而不是任意 GMM 的特例。
...
- K 均值与 GMM/EM 的软硬分配区别，以及两者对应所需的球形协方差等条件。
- 肘部法则定 $K$。
