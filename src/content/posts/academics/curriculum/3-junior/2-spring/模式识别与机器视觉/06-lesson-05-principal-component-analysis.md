---
title: "第五讲 · 主成分分析 PCA 与 LDA"
description: "从最大方差与最小误差两个视角理解 PCA，掌握协方差特征值分解、计算步骤及与 LDA 的区别"
date: 2026-08-22
tags: ["模式识别与机器视觉", "数学"]
---

PCA 是无监督降维的代表。最大方差和最小误差看起来是两个不同的目标，最后都归结为协方差矩阵的特征值分解。

## 一、动机

把高维相关数据压成低维，又尽量不丢信息。做法：找数据"铺得最开（方差最大）"的方向投影。方差大的方向区分度高、信息多；方差小的方向点挤成团，往往是噪声，可丢弃。

![把二维数据投影到方差最大的主轴](/images/pattern-recognition-vision/pca-projection.png)

*图：PCA 不关心类别标签，只寻找整批数据延展最明显的正交方向。*
...
要 $J$ 最小，就丢掉最小的 $D-M$ 个特征值、保留最大的 $M$ 个。这与最大方差视角得到的选择一致。
...
> 一个常见练习是给出 2×2 协方差矩阵，计算特征值（$\det(\mathbf{S} - \lambda\mathbf{I}) = 0$）和特征向量。
...
3. 只利用均值与协方差这类二阶统计量，可能忽略非线性流形和更高阶的分布结构。

## 六、PCA vs LDA

![PCA 与 LDA 选择投影方向的差别](/images/pattern-recognition-vision/pca-vs-lda.png)
...
PCA 希望投影后保留尽可能多的总体变化；LDA 则利用标签，让类间尽量分开、类内尽量紧凑。
