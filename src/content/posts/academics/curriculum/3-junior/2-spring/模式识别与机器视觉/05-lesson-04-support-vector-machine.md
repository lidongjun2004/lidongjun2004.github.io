---
title: "第四讲 · 支持向量机 SVM"
description: "最大间隔、±1 标签、规范化与基本型、拉格朗日对偶、KKT 与支持向量，以及软间隔"
date: 2026-08-22
tags: ["模式识别与机器视觉", "数学"]
---
...
![小间隔与大间隔分类面的对比](/images/pattern-recognition-vision/svm-small-large-margin.png)
...
这是**凸二次规划**：线性可分时最优解是全局最优；线性不可分时，硬间隔约束没有可行解。
目标函数对 $\mathbf w$ 严格凸，因此最优法向量唯一；在退化情形下，偏置 $b$ 不一定唯一。
...
互补松弛条件直接解释了支持向量：对每个点，要么 $a_n = 0$（对 $\mathbf{w}$ 无贡献），要么 $t_n y(\mathbf{x}_n) = 1$（点恰在间隔边界上）。
...
**手算步骤**：用 $\sum_n a_n t_n = 0$ 减少未知数 → 写 $\mathbf{w} = \sum_n a_n t_n \mathbf{x}_n$ → 对每个支持向量列 $t_n(\mathbf{w}^{\mathsf{T}}\mathbf{x}_n + b) = 1$ → 解方程组。
...
$\xi_n$ 的含义：$\xi_n = 0$ 时满足间隔要求；$0 < \xi_n < 1$ 时进入间隔但仍在正确一侧；$\xi_n = 1$ 时落在分类超平面上；$\xi_n > 1$ 时被分错。

目标加惩罚项：

$$\min_{\mathbf{w}, b, \xi}\ \frac{1}{2}\|\mathbf{w}\|^2 + C \sum_{n=1}^{N} \xi_n \quad \text{s.t.}\quad t_n\, y(\mathbf{x}_n) \ge 1 - \xi_n,\ \xi_n \ge 0$$

$C$ 控制违规惩罚的强度：$C$ 较大时更强调减少训练集违规，正则化较弱；$C$ 较小时允许更多违规，正则化较强。它不直接保证泛化性，具体值仍需用验证集选择。

软间隔对偶与硬间隔几乎相同，仅 $a_n$ 多了上界：

$$0 \le a_n \le C, \qquad \sum_{n=1}^{N} a_n t_n = 0$$

软间隔里，$0 < a_n < C$ 的点满足 $\xi_n = 0$，恰在间隔边界；凡是 $\xi_n > 0$
的点都满足 $a_n = C$，它们进入了间隔或被错分。反过来不一定成立：退化情形下，
$a_n = C$ 的点也可能仍在边界上，因此不能把 $a_n=C$ 与“必然越界”画等号。
...
- 两点**手算例子**全流程。
- 软间隔：松弛变量、惩罚 $C$ 的权衡、$0 \le a_n \le C$，以及 $\xi_n>0\Rightarrow a_n=C$。
