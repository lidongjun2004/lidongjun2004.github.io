---
title: "第三讲 · 概率密度估计与最大似然估计"
description: "两步贝叶斯决策、MLE 五步法、常见分布的 MLE 计算、均匀分布的约束，以及有偏无偏与 N-1 修正"
date: 2026-08-22
tags: ["数学"]
---

贝叶斯决策要用到先验和类条件密度，但现实里它们往往**未知**，只能从样本估计。我在这一节讲怎么从样本把密度参数估出来——最大似然估计（MLE）。
...
可以概括为：MLE 的方差估计有偏且偏小，期望为 $\dfrac{N-1}{N}\sigma^2$；改用 $N-1$ 作分母后得到无偏估计。

## 复习要点

- 两步贝叶斯决策、估计方法三分类。
- **MLE 五步法**，会对高斯 / 伯努利 / 指数 / 泊松完整推导 + 数值计算。
- **均匀分布**：先考虑参数约束，再由似然函数的单调性得到 $\hat{\theta} = \max_i x_i$。
- **有偏无偏**：高斯方差 MLE 偏小、期望 $\frac{N-1}{N}\sigma^2$，以及 $N-1$ 修正与自由度解释。
