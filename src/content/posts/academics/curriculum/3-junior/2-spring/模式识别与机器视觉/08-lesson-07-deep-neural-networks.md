...
课件用图像描述任务把 CNN、RNN 和 Attention 串在了一起。预训练 CNN 先保留一组空间特征
$z_1,\dots,z_L$，每个 $z_i$ 对应图像中的一个区域。没有 Attention 时，这组特征先经 MLP
压成 RNN 的初始隐藏状态 $h_0$；后续各词只能沿状态链继续使用这份固定摘要，长描述容易形成信息瓶颈。

加入 Attention 后，第 $t$ 步会根据上一时刻的隐藏状态重新分配区域权重：

$$
e_{t,i}=f_{\mathrm{att}}(h_{t-1},z_i),\qquad
\alpha_{t,i}=\frac{\exp(e_{t,i})}{\sum_j\exp(e_{t,j})},\qquad
c_t=\sum_i\alpha_{t,i}z_i.
$$

生成“person”时可以多看人物区域，生成“hat”时再转向帽子区域。这里的关键不是某个固定网络名称，
而是**每一步都从 CNN 空间特征中取一个不同的上下文向量 $c_t$**，从而缓解固定初始摘要的信息瓶颈。
