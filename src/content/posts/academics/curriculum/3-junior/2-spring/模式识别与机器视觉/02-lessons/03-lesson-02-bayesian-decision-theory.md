...
贝叶斯决策回答一个问题：**在类的概率分布完全已知时，怎么判类才最优**。后面的计算主要围绕后验概率、错误率和条件风险展开。
...
公式中的关系是：后验概率由类条件密度与先验概率的乘积得到，再用总体密度归一化。
...
其中 $p_1(e) = \int_{\mathcal{R}_2} p(x \mid \omega_1)\, dx$ 是第一类样本被判进第二类区域的错误，$p_2(e)$ 同理。两类问题的贝叶斯决策边界满足

$$
p(x\mid\omega_1)P(\omega_1)=p(x\mid\omega_2)P(\omega_2).
$$

只有先验概率相等时，它才简化为两条类条件密度曲线的交点。

逐点条件错误率 $P(e \mid x) = 1 - \max_i P(\omega_i \mid x)$，总错误率 $P(e) = E[P(e \mid x)]$。贝叶斯决策让每个观测点的条件错误率都最小，因而达到给定分布下可能的最低总错误率，称为**一致最优决策**。
...
**这与最小错误率的结论（判健康）正好相反**：漏诊代价是误诊的 6 倍，所以宁可误诊也不敢漏诊。复习时要能解释结论为什么会随损失矩阵改变。
...
- **0-1 损失把最小风险退化为最小错误率**，这是两种决策准则的关系。
