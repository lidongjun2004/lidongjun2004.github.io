...
我在下面既记录这些原始要求，也区分“题目要求”与“我们实际完成的内容”，避免把二者混成同一事实。
...
原题写的是“构建包含自己照片的数据集”；我们实际提交时把开放题具体化为汉堡、馅饼和寿司三类食品图像分类。我们由此完成了自建分类数据与应用链路，但报告没有证明数据满足“包含自己照片”这一字面要求。数据按 `ImageFolder` 组织为 `train/<class>` 与 `val/<class>`，预处理脚本完成：
