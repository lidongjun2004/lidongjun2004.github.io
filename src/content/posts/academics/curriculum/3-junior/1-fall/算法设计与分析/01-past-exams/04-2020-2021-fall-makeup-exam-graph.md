...
这一部分来自 2021 年 3 月 8 日补考原卷及一页课程答案。选择、判断题有原答案；算法题没有随卷答案，我在下面给出推导。
...
原卷只写了“递增—递减—递增”，没有说明相邻元素能否相等，也没有另行声明内部最小值必为负。我按示例表达的意图，把下面的 $O(\log n)$ 算法解释为：**以下按严格单调理解**，并假设内部谷值小于 0。若允许中途出现平台，仅靠当前元素与相邻元素的大小关系可能无法判断谷底在哪一侧；缺少进一步条件时，可线性扫描求最小值，时间 $O(n)$。
...
每轮把区间减半，时间 $O(\log n)$、空间 $O(1)$。严格单调和负谷值是我在解法中采用的解释，不是原卷明写的额外条件。
