...
description: "计算机组成实验 5 的提交记录，梳理单周期 CPU 电路、子模块和调试材料。"
date: 2026-08-27
tags: ["作业"]
---

> 本实验没有单独题目文档。课程介绍把对应阶段描述为“完成支持 7 条指令的单周期 CPU 设计”；具体提交以 `单周期CPU.circ` 为准，不根据测试工具反推额外指令要求。
...
评测工具 README 标明其用途是导出 Logisim 结果、对拍电路输出，以及把汇编交给 MARS 作为参考模型。该工具不是 CPU 主提交的一部分，文章没有把它的功能算作实验要求。
