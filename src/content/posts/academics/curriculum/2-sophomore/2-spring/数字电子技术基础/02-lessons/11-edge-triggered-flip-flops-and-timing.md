...
![正边沿 D 触发器的建立时间与保持时间窗口](/images/academics/digital-electronics/lessons/setup-hold-window.png)

虚线围出的就是采样边沿两侧的禁变窗口：边沿前留出 $t_{su}$，边沿后继续保持 $t_h$。只有 D 在这段时间内稳定，触发器才有可靠的采样条件。
