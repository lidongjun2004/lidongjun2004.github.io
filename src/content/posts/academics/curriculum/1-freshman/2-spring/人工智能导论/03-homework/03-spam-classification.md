...
课件给出的 `SMSSpamCollection` 数据共有约 5574 条记录，标签为 `spam` 或 `ham`。源目录中保留了数据集和李东骏提交的 `垃圾短信分类.ipynb`，没有发现单独的报告文件，因此下文只能按 Notebook 还原实现与输出。

<details class="exam-answer">
<summary>查看李东骏的 Notebook 实现、输出与复盘</summary>
...
手写 40 条短信也不是从同一数据分布独立抽样，且“看起来像 ham 的 spam”之类标签由作者预先主观设定，只适合做挑战样例，不能替代标准测试集。很多诈骗短信和正规营销短信在词面上本来就非常相似，单靠词袋特征很难稳定识别来源真实性。
