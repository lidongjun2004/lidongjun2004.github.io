...
这不是一份完整题单。源目录只保留了 `E5 - Solution.pdf` 和 Lee 当时 A～H 的答题代码，没有原始题面；解析册也多次直接引用“题目描述”，没有重印完整输入、输出与数据约束。

因此，下面折叠外只记录解析册能够直接确认的题名、难度、考点和任务轮廓，不补造缺失条件；折叠内整理解析册思路与留存代码的核心实现。I、J 没有个人代码，只记录解析册方案。若要重新提交这些题，必须先找到原 OJ 题面核对全部约束。
...
<summary>查看解析与 Lee 的核心代码</summary>
...
Lee 的留存代码把“角分类”和“等腰分类”拆成 `f1`、`f2` 两个函数，再用二维下标表保存 7 组三元组合。上面合并为一段是为了阅读方便，输出字符串沿用解析册。
...
解析册给出两条路线：对费用函数求导后二分导数零点；或利用费用函数先减后增的单峰性质直接三分。Lee 留存的是三分版本：
...
Lee 的留存代码用数组按镜像关系逐层构造，属于解析册的第二种方案。位运算版本更短，但 `1u << n` 要求 $n$ 小于 `unsigned` 位宽，具体上限仍需原题确认。
...
<details class="exam-answer">
<summary>查看解析与 Lee 的核心思路</summary>
...
Lee 的留存代码也先分别处理 $m$、$n$，再组合三个计数，但把多个小规模情况展开成了分支。这里展示解析册更直接的递推式。
...
解析册指出二维数组方案直观但空间很大，而且只适合这种可直接复制的图案；递归描述更通用。Lee 的 E5 目录没有保留 I 题个人代码。
...
- A～H 另有 Lee 的代码文件，可用于交叉核对实现轮廓；
- I、J 只有解析册示例；
- 原始题面、完整样例、限制与评测环境均未保留；
- 本页故意不补写“输入格式”“输出格式”“数据范围”，也不把它放进真题目录。
